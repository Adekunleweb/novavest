...
let resend = null;
let fromEmail = process.env.RESEND_FROM_EMAIL || 'ApexCrestVest <noreply@apexcrestvest.com>';
...
// Base email template with ApexCrestVest gold/navy branding
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#112240;border-radius:16px;overflow:hidden;max-width:600px;border:1px solid #1e3a5f;">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#0a1628,#1a2f4e);padding:30px 40px;text-align:center;border-bottom:2px solid #d4af37;">
              <h1 style="margin:0;color:#d4af37;font-size:28px;letter-spacing:1px;">ApexCrestVest</h1>
          ...
          <tr>
            <td style="padding:25px 40px;background:#0a1628;border-top:1px solid #1e3a5f;">
              <p style="margin:0;color:#5c6f8a;font-size:12px;text-align:center;line-height:1.6;">
                This is an automated message from ApexCrestVest. Please do not reply to this email.<br>
                &copy; 2026 ApexCrestVest. All rights reserved.
              </p>
            </td>
          </tr>
        </table>
...
async function notifySignup(user) {
  const content = `
    <h2 style="margin:0 0 20px;color:#e6f1ff;font-size:22px;">Welcome to ApexCrestVest, ${user.full_name}! 🎉</h2>
...
async function notifyPasswordReset(user, resetUrl) {
  const content = `
    ...
    <p style="color:#a8b2d1;font-size:15px;line-height:1.7;">
      Hello ${user.full_name}, we received a request to reset your ApexCrestVest account password.
    </p>
    <p style="color:#a8b2d1;font-size:15px;line-height:1.7;">
  ...
  return sendEmail(user.email, 'Password Reset Request — ApexCrestVest 🔐', content);
}
...
async function notifyPasswordChanged(user) {
  const content = `
    ...
    <p style="color:#a8b2d1;font-size:15px;line-height:1.7;">
      Hello ${user.full_name}, your ApexCrestVest account password has been successfully changed.
    </p>
  ...
  return sendEmail(user.email, 'Password Changed — ApexCrestVest ✅', content);
}
